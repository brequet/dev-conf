...
pub async fn install_package(package: &Package) -> Result<()> {
    ...
    if let Some(before) = &package.before {
        if dry_run {
            tracing::info!("[dry-run] Would run before hook for {}", package.id);
        } else {
    ...
    // Install (dry-run is handled by the caller; if we get here, we do the real install)
    if !dry_run {
    ...
    if let Some(after) = &package.after {
        if dry_run {
            tracing::info!("[dry-run] Would run after hook for {}", package.id);
        } else {
